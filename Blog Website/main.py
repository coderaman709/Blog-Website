...
def home():
    ...
    if 'username' in session:
        ...
        return render_template("home.html", username=session['username'], posts=posts,  theme=theme, post=post)
...
@app.route("/<string:link>/<string:password>/edit", methods= ['GET', 'POST'])
def editBlog(link, password):
    data1 = Blogs.query.filter_by(link=link).first()
    data2 = Users.query.filter_by(password=password).first()
    if (request.method == 'POST' and data2.password==password):
        img = request.form.get("img")
        title = request.form.get("title")
        category = request.form.get("category")
        blog = request.form.get("blog")
        ost = Blogs.query.filter_by(link=data1).first()
